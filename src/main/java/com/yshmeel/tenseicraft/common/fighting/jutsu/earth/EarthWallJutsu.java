...
public class EarthWallJutsu extends Jutsu {
    ...
    public void registerEntities(RegistryEvent.Register<EntityEntry> event) {
        event.getRegistry().register(
            EntityEntryBuilder.<Entity>create()
                    .entity(EarthWallEntity.class)
                    .id(new ResourceLocation("tenseicraft:earth_wall_entity"), ModInfo.entityIds++)
                    .name("EarthWall")
                    .tracker(64, 3, true)
                    .build()
        );
    ...
    public boolean throwJutsu(EntityPlayer fromPlayer, EntityPlayer toPlayer, World world) {
        if(fromPlayer != null) {
            ...
            EarthWallEntity jutsuEntity = new EarthWallEntity(fromPlayer.getEntityWorld(), fromPlayer);
    ...
    static class EarthWallEntity extends EntityThrowable implements IProjectile {
        private int ticksInGround = 0;
        private float[][][] savedWallPos = null;
        private boolean start = false;

        public EarthWallEntity(World worldIn)
        {
            super(worldIn);
        ...
        public EarthWallEntity(World worldIn, double x, double y, double z)
        {
            this(worldIn);
        ...
        public EarthWallEntity(World worldIn, EntityLivingBase player)
        {
                ...
                for(float[][] rowWallCoords : wallPos) {
                    for(float[] pieceRowWall : rowWallCoords) {
                        ...
                        if(this.getEntityWorld().getBlockState(pos).equals(Blocks.AIR.getDefaultState())) {
                            this.getEntityWorld().setBlockState(pos, Blocks.DIRT.getDefaultState());
                        }
                    }
                }
            }
            ...
            if(this.start) {
                ...
                if(this.ticksInGround >= 240) {
                    ...
                    for(float[][] rowWallCoords : wallPos) {
                        for(float[] pieceRowWall : rowWallCoords) {
                            BlockPos pos = new BlockPos(pieceRowWall[0], pieceRowWall[1], pieceRowWall[2]);
                            if(this.getEntityWorld().getBlockState(pos).equals(Blocks.DIRT.getDefaultState())) {
